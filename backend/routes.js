...
router.post("/reviews", async (req, res) => {
  ...
  try {
    ...
    if (tipoUsuario === "freelancer") {
      ...
      // Obtener empresa
      const empresaResults = await getEmpresaByUserId(id_calificado);
      if (empresaResults.length === 0) {
        return sendError(res, 404, "No se encontró la empresa");
      }
      const id = empresaResults[0].id_usuario;

      // Obtener usuario
      const usuarioResults = await getUserById(id);
      if (usuarioResults.length === 0) {
      ...
      if (!freelancer || freelancer.length === 0) {
        return res.status(404).json({message: "El freelancer no existe."});
      }
    ...
    await connection.query(
        "INSERT INTO resena (id_usuario, tipo_calificado, id_calificado, calificacion, comentario) VALUES (?, ?, ?, ?, ?)",
